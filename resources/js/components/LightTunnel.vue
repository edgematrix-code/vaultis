...
type FlowDirection = 'outward' | 'inward';
...
const syncUniforms = () => {
    ...
    u.uLightMode.value = props.lightMode ? 1.0 : 0.0;
...
onMounted(() => {
    ...
    // WebGL2 is required by the shader; bail out where unsupported.
    if (!el) return;

    if (!document.createElement('canvas').getContext('webgl2')) {
        console.warn(
            '[LightTunnel] WebGL2 is not available in this browser; skipping the hero animation.',
        );
    ...
    const program = new Program(gl, {
        vertex,
        fragment,
        // ogl only knows the uniforms supplied here — defaults mirror the
        // original component, then syncUniforms() applies the props.
        uniforms: {
            iTime: { value: 0 },
    ...
    const loop = (t: number) => {
        ...
        renderer.render({ scene: mesh });
        if (ctx) ctx.raf = requestAnimationFrame(loop);
    };

    const tryStart = () => {
        if (ctx && isVisible && isPageVisible && ctx.raf === 0)
    ...
    ctx.intersectionObserver = intersectionObserver;
    ...
    ctx.onVisibilityChange = onVisibilityChange;
...
onBeforeUnmount(() => {
    ...
    if (ctx.raf !== 0) cancelAnimationFrame(ctx.raf);

    ctx.resizeObserver.disconnect();
    ctx.intersectionObserver.disconnect();
    document.removeEventListener('visibilitychange', ctx.onVisibilityChange);
    canvas.removeEventListener('mousemove', ctx.onMouseMove);
    canvas.removeEventListener('mouseleave', ctx.onMouseLeave);
...
<template>
    <!-- Position/sizing comes from the consumer via class (e.g. `absolute inset-0`) -->
    <div ref="container" class="h-full w-full overflow-hidden"></div>
</template>
